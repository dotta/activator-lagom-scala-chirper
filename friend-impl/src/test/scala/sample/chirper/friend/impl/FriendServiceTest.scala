...
class FriendServiceTest {
  ...
  def shouldBeAbleToCreateUsersAndConnectFriends() {
    withServer(defaultSetup, server => {
      ...
      friendService.addFriend().invoke("usr1", FriendId(usr2.userId)).toCompletableFuture().get(3, SECONDS)
      friendService.addFriend().invoke("usr1", FriendId(usr3.userId)).toCompletableFuture().get(3, SECONDS)

      val fetchedUsr1 = friendService.getUser().invoke("usr1", NotUsed).toCompletableFuture().get(3,
          SECONDS)
      ...
      eventually(FiniteDuration(10, SECONDS), () => {
        val followers = friendService.getFollowers().invoke("usr2", NotUsed)
